...
# displays a set of circles, specified by three arrays containing x- & y-coordinates, as well as circle radius;
# optionally a score can be specified that is visulalized via the circle color (brighter color == higher score)
...
def Ransac(edge_points):
    ...
    while current_iter < max_iters and current_iter < N:
        ...
        sample_indices = np.random.randint(0, edge_points.shape[1], size=3)
        initial_samples = edge_points[:, sample_indices]
        ...
        distance_to_center = np.linalg.norm(edge_points-center[:, np.newaxis], axis=0)

        # TODO: compare with model radius to determine if points are inliers or not (within margin)
        inlier_mask = np.abs(distance_to_center-radius) < distance_threshold
        # inlier_mask = np.broadcast_to(inlier_mask, (2, inlier_mask.size))
        assert(inlier_mask.any())
    ...
    new_edgepoints = edge_points[:, ~inlier_mask_best]
    assert(len(new_edgepoints))
